...
function getVisualizedMesh( linearData, year, countries, exportCategories, importCategories ){
	...
	for( i in bin ){
		...
		if( (useImporter || useExporter) && (relevantExportCategory || relevantImportCategory) ){
            ...
            //size of the line
            var particleCount = Math.floor(set.v *20 / set.lineGeometry.vertices.length ) + 1;
			...
			if( exporterCountry == selectedCountry ){				
				selectedCountry.summary.exported[set.wc] += parseInt(set.v);
				selectedCountry.summary.exported.total += parseInt(set.v);
			}		
			if( importerCountry == selectedCountry ){
				selectedCountry.summary.imported[set.wc] += parseInt(set.v);
				selectedCountry.summary.imported.total += parseInt(set.v);
			}

			if( importerCountry == selectedCountry || exporterCountry == selectedCountry ){
				selectedCountry.summary.total += parseInt(set.v);
			}
